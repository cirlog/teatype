...
const fallbackReducer = (state: FallbackState, action: FallbackAction): FallbackState => {
    switch (action.type) {
        ...
        case 'FALLBACK':
            return { attempts: MAX_RETRY_ATTEMPTS, overridePath: FALLBACK_ICON_PATH };
        default:
            return state;
    }
};
 ...
 */
const TTIcon = memo<iTTIconProps>(function TTIcon({
    className,
    filled = false,
    id,
    animated = false,
    ...
    const handleLoadError = useCallback<ErrorCallback>(
        (error) => {
            ...
            } else if (fallback.attempts < MAX_RETRY_ATTEMPTS) {
